...
const SelectedTechs = ({ selectedTechs, setSelectedTechs }: selectedProps) => {
  ...
  return (
    <div className="w-[284px] max-h-fit rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <div>
        <h2 className="text-[15px] font-semibold text-gray-900">Your Stack</h2>
      ...
      <div className="mt-4 rounded-xl">
        {selectedTechs.length === 0 ? (
          <p className="text-[12px] text-[#9aa9c2]">Your stack is empty</p>
        ) : (
          selectedTechs.map((selected) => (
            <div className="max-w-[100%]" key={selected.id}>
              <Selecteditem
      ...
      {selectedTechs.length === 0 ? null : (
        <div>
          <button
            onClick={handleRemoveAllButton}
            type="button"
            className="font-semibold text-[14px] text-[#D82C20] text-center border-2 border-[#ED8C85] rounded-[8px] px-20 py-2 mt-8 hover:text-white hover:bg-[#D82C20]"
          >
            Remove All
          </button>
        </div>
